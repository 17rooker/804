...
{
    // 此处替换为原upDataUi中的**耗时逻辑**（如数据解析、状态计算）
    FrameDataAnalysis analy;
    STPackage pack;
    pack.channelId="serial_E";
    pack.eDataType=EDataType::E_ServerControl_BrakeRelease_A5;
    pack.channelType=EChannelType::Serial;
    pack.ulDataLen=data.size();
    pack.baDataRecv=data;
    STParamInfo param;
...
void LaunchFrameDialog::setupUI()
{
    ...
    mergeLayout->addWidget(lblFrameLen, 0, 0);

    QLabel *lblCheck = new QLabel("校验结果");
    lblCheck->setAlignment(Qt::AlignCenter);
    mergeLayout->addWidget(lblCheck, 0, 1);
    ...
    mergeLayout->addWidget(leFrameLen, 1, 0);
    ...
    mergeLayout->addWidget(leCheck, 1, 1);
