...
void LaunchProcessDialog::setupUI()
{
    ...
    // 4. 三窗口表决区域
    // ==========================================
    QVBoxLayout *voteLayout = new QVBoxLayout();
    setupVoteUI(voteLayout);
    mainLayout->addLayout(voteLayout, 1);
}
// ═══════════════════════════════════════════════════════════════════════════
//  三窗口表决
// ═══════════════════════════════════════════════════════════════════════════
static void setVoteLed(StyledLedLabel *led, int ones, int total) {
    if (!led) return;
    if (ones == total)       // 全1 → 绿
        led->setStyleSheet("background-color: #00CC00; border-radius: 8px; border: 1px solid white;");
    else if (ones == 0)      // 全0 → 暗绿
...
void LaunchProcessDialog::setupVoteUI(QVBoxLayout *parent)
{
    QGroupBox *box = new QGroupBox("三窗口表决");
    QVBoxLayout *vb = new QVBoxLayout(box);

    // 模式显示
    QHBoxLayout *modeRow = new QHBoxLayout();
    modeRow->addWidget(new QLabel("模式："));
    m_lblVoteMode = new QLabel("---");
    m_lblVoteMode->setStyleSheet("background: black; color: #00FF00; padding: 4px; font-weight: bold;");
    m_lblVoteMode->setFixedHeight(28);
    modeRow->addWidget(m_lblVoteMode);
    modeRow->addStretch();
    ...
    struct VoteItem {
        QString label;
        QString key;   // 用于查找 m_voteLedMap
        int row, col;
    };

    QList<VoteItem> items = {
        {"电缆连接",    "表决_电缆连接", 0, 0},
        {"供气状态",    "表决_供气状态", 0, 1},
        {"解锁状态",    "表决_解锁状态", 0, 2},
        {"火引爆",      "表决_火引爆",   0, 3},
        {"牵制释放准备好","表决_牵制释放准备好", 1, 0},
        {"转发释放好",  "表决_转发释放好", 1, 1},
        {"5VK",         "表决_5VK",      1, 2},
        {"开锁状态",    "表决_开锁状态", 1, 3},
        {"机构1释放好", "表决_机构1释放好", 2, 0},
        {"机构2释放好", "表决_机构2释放好", 2, 1},
        {"机构3释放好", "表决_机构3释放好", 2, 2},
        {"机构4释放好", "表决_机构4释放好", 2, 3},
        {"总释放好",    "表决_总释放好",  3, 0},
    };

    for (auto &it : items) {
        auto *led = new StyledLedLabel(this);
        led->setFixedSize(20, 20);
        m_voteLedMap[it.key] = led;
        QVBoxLayout *vl = new QVBoxLayout();
        vl->setAlignment(Qt::AlignCenter);
        vl->setSpacing(2);
        vl->addWidget(led, 0, Qt::AlignCenter);
        QLabel *lb = new QLabel(it.label);
        lb->setFont(QFont("SimHei", 9));
        lb->setAlignment(Qt::AlignCenter);
        vl->addWidget(lb);
        QWidget *cw = new QWidget;
        cw->setLayout(vl);
        grid->addWidget(cw, it.row, it.col);
    }
...
// ── 表决辅助：获取某个字段在3个通道中的值(默认0) ──
static quint8 getFieldVal(const STParamInfo &p, const QString &key) {
    auto it = p.mapParams.find(key);
    if (it != p.mapParams.end()) {
        bool ok = false;
        quint8 v = it.value().varParaValue.toUInt(&ok);
        if (ok) return v;
    }
    return 0;
}

// ── 表决辅助：统计某字段在各通道中特定位为1的个数 ──
static int countBits(const STParamInfo &pE, const STParamInfo &pF, const STParamInfo &pG,
                     const QString &key, int bit) {
    int cnt = 0;
    if ((getFieldVal(pE, key) >> bit) & 1) cnt++;
    if ((getFieldVal(pF, key) >> bit) & 1) cnt++;
    if ((getFieldVal(pG, key) >> bit) & 1) cnt++;
    return cnt;
}
...
void LaunchProcessDialog::updateVoting()
{
    ...
    auto getMode = [&](const STParamInfo &p) -> QString {
        quint8 v = getFieldVal(p, "WorkMode");
        switch (v) {
        case 0xAA: return "测试模式";
        case 0xBB: return "手动模式";
        case 0xCC: return "自动模式";
        default: return QString();
        }
    };
    QString mE = getMode(m_paramE), mF = getMode(m_paramF), mG = getMode(m_paramG);
    if (m_lblVoteMode) {
        if (!mE.isEmpty() && mE == mF && mF == mG)
            m_lblVoteMode->setText(mE + "（一致）");
        else if (mE.isEmpty() && mF.isEmpty() && mG.isEmpty())
            m_lblVoteMode->setText("---");
        else
            m_lblVoteMode->setText("模式错误");
    }

    // ── 状态灯表决 ──
    auto vote = [&](const QString &ledKey, int cnt) {
        auto it = m_voteLedMap.find(ledKey);
        if (it != m_voteLedMap.end()) setVoteLed(it.value(), cnt, 3);
    };

    auto voteOr = [&](const QString &ledKey, const QStringList &keys, int bit) {
        int cnt = 0;
        for (const auto &k : keys)
            cnt += countBits(m_paramE, m_paramF, m_paramG, k, bit);
        // 归一化到 0..3 (any channel with any match counts as 1)
        cnt = qMin(3, cnt > 0 ? 1 + (cnt > 1 ? 1 : 0) + (cnt > 2 ? 1 : 0) : 0);
        // Actually simpler: just count channels where ANY key/bit is 1
        return cnt;
    };

    // 电缆连接: SolenoidValveStatus1-4 B6 / OE_KJ1-5 B6
    auto cableVote = [&]() {
        int c = 0;
        for (auto &p : {&m_paramE, &m_paramF, &m_paramG}) {
            bool any = false;
            for (int m = 1; m <= 4; m++) {
                if ((getFieldVal(*p, QString("SolenoidValveStatus%1").arg(m)) >> 6) & 1) any = true;
                if ((getFieldVal(*p, QString("OE_KJ%1").arg(m==3?4:m==4?5:m==1?1:2)) >> 6) & 1) any = true;
            }
            if (any) c++;
        }
        vote("表决_电缆连接", c);
    ...
    // 供气: SolenoidValveStatus1-4 B4 / OE_KJ1-5 B4
    auto supplyVote = [&]() {
        int c = 0;
        for (auto *p : {&m_paramE, &m_paramF, &m_paramG}) {
            bool any = false;
            for (int m = 1; m <= 4; m++) {
                if ((getFieldVal(*p, QString("SolenoidValveStatus%1").arg(m)) >> 4) & 1) any = true;
                int oej = (m==3)?4:(m==4)?5:(m==1)?1:2;
                if ((getFieldVal(*p, QString("OE_KJ%1").arg(oej)) >> 4) & 1) any = true;
            }
            if (any) c++;
        }
        vote("表决_供气状态", c);
    ...
    // 解锁: SolenoidValveStatus1-4 B0/B1 / OE_KJ1-5 B0/B1
    auto unlockVote = [&]() {
        int c = 0;
        for (auto *p : {&m_paramE, &m_paramF, &m_paramG}) {
            bool any = false;
            for (int m = 1; m <= 4; m++) {
                quint8 sv = getFieldVal(*p, QString("SolenoidValveStatus%1").arg(m));
                if ((sv & 0x01) || (sv & 0x02)) any = true;
                int oej = (m==3)?4:(m==4)?5:(m==1)?1:2;
                quint8 oj = getFieldVal(*p, QString("OE_KJ%1").arg(oej));
                if ((oj & 0x01) || (oj & 0x02)) any = true;
            }
            if (any) c++;
        }
        vote("表决_解锁状态", c);
    ...
    // 火引爆: InitiatorDetonateRelayStatus B0-3 / OE_KJ10 B0-3
    auto fireVote = [&]() {
        int c = 0;
        for (auto *p : {&m_paramE, &m_paramF, &m_paramG}) {
            quint8 ids = getFieldVal(*p, "InitiatorDetonateRelayStatus");
            quint8 oj10 = getFieldVal(*p, "OE_KJ10");
            if ((ids & 0x0F) || (oj10 & 0x0F)) c++;
        }
        vote("表决_火引爆", c);
    };
    fireVote();

    // 牵制释放准备好: ReleasePermitAndPowerStatus B7 / ...
    auto readyVote = [&]() {
        int c = 0;
        for (auto *p : {&m_paramE, &m_paramF, &m_paramG}) {
            quint8 rp = getFieldVal(*p, "ReleasePermitAndPowerStatus");
            if (rp & 0x80) c++;
        }
        vote("表决_牵制释放准备好", c);
    };
    ...
    // 转发释放好: Mechanism1_2ReleaseStatus B6-7 / ...
    auto forwardVote = [&]() {
        int c = 0;
        for (auto *p : {&m_paramE, &m_paramF, &m_paramG}) {
            quint8 m12 = getFieldVal(*p, "Mechanism1_2ReleaseStatus");
            quint8 m34 = getFieldVal(*p, "Mechanism3_4ReleaseStatus");
    ...
    // 5VK: from 5V status fields
    auto v5k = [&]() {
        int c = 0;
        for (auto *p : {&m_paramE, &m_paramF, &m_paramG}) {
            quint8 d5 = getFieldVal(*p, "Digital5V1");
            if (d5 > 0) c++;
        }
        vote("表决_5VK", c);
    };
    ...
    // 开锁: SolenoidValveStatus1-4 B2 / OE_KJ1-5 B2
    auto lockVote = [&]() {
        int c = 0;
        for (auto *p : {&m_paramE, &m_paramF, &m_paramG}) {
            bool any = false;
            for (int m = 1; m <= 4; m++) {
                if ((getFieldVal(*p, QString("SolenoidValveStatus%1").arg(m)) >> 2) & 1) any = true;
                int oej = (m==3)?4:(m==4)?5:(m==1)?1:2;
                if ((getFieldVal(*p, QString("OE_KJ%1").arg(oej)) >> 2) & 1) any = true;
            }
            if (any) c++;
        }
        vote("表决_开锁状态", c);
    ...
    for (int m = 1; m <= 4; m++) {
        int c = 0;
        QString ledKey = QString("表决_机构%1释放好").arg(m);
        for (auto *p : {&m_paramE, &m_paramF, &m_paramG}) {
            quint8 m12 = getFieldVal(*p, "Mechanism1_2ReleaseStatus");
            quint8 m34 = getFieldVal(*p, "Mechanism3_4ReleaseStatus");
            bool ok = false;
            if (m <= 2) ok = (m12 >> (m == 1 ? 6 : 7)) & 1;
            else        ok = (m34 >> (m == 3 ? 6 : 7)) & 1;
            if (ok) c++;
        }
        vote(ledKey, c);
    }

    // 总释放好：4个机构均释放好，且3个通道一致
    {
        int c = 0;
        for (auto *p : {&m_paramE, &m_paramF, &m_paramG}) {
            quint8 m12 = getFieldVal(*p, "Mechanism1_2ReleaseStatus");
            quint8 m34 = getFieldVal(*p, "Mechanism3_4ReleaseStatus");
            bool allOk = ((m12 >> 6) & 1) && ((m12 >> 7) & 1) && ((m34 >> 6) & 1) && ((m34 >> 7) & 1);
            if (allOk) c++;
        }
        vote("表决_总释放好", c);
    }
